...
from datetime import datetime, timedelta  # ← 여기 timedelta 추가
...
def load_price_data(ticker: str, start: str, end: str):
    ...
    # [중요] 멀티인덱스 컬럼을 제거하여 구조를 단순화합니다.
    # yfinance 버전 이슈로 컬럼이 ('Close', '035720.KS') 형태로 올 때 이를 'Close'로 바꿉니다.
...
# ★ 여기서 end_date에 하루를 더해서 31일까지 포함되도록 처리 ★
df = load_price_data(
    ticker,
...
with tab4:
    ...
    if levels:
        ...
        st.write([round(l, -2) for l in levels])
    else:
